...
export const trackVisitorEvent = async (event: any, input: TrackVisitorEventInput) => {
  ...
  const ip = requestGeo?.ip || null
  const country = normalizeValue(input.country) || requestGeo?.country || null
  const region = normalizeValue(input.region) || requestGeo?.region || null
  const city = normalizeValue(input.city) || requestGeo?.city || null
