...
export async function listQingpuTrialProducts() {
  ...
  return rows
    .filter((product: { price: number }) => Number(product.price) === 0)
    .map((product: { id: number, name: string, type: string }) => ({
...
export async function beginQingpuTrialFulfillment(orderId: string, adminId: number | null) {
  ...
  if (current.state === 'processing') {
    ...
    await db.update(orders).set({ status: ORDER_STATUS.FAILED }).where(and(
      eq(orders.id, orderId),
  ...
  return { order: toTrialOrder({ ...row, metaData: mergeTrialMeta(row.metaData, next) }), claimed: true }
}
...
export async function completeQingpuTrialOrder(orderId: string) {
  ...
  await db.update(orders).set({
    ...
    metaData: mergeTrialMeta(row.metaData, { ...current, state: 'approved', lastError: undefined }),
  }).where(eq(orders.id, orderId))
}
