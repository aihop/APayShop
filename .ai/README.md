...
3. 只修改 claims；发现需要扩围、租约冲突或其他会话改动时停止，不得用更宽路径或新任务绕过重叠租约。
4. 完成后执行：
...
- 需要扩围：停止修改，先按当前现场安全收尾，再调整契约并重新取得用户确认。
- 租约过期且 claims 已修改：保留现场并报告；当前脚本没有 renew/resume/recover，不得手改运行态后继续。
