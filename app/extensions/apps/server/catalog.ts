...
const TABLE = 'ext_apps_catalog'
const STATUS_DRAFT = 10
const STATUS_PUBLISHED = 20
const VALID_TYPES = new Set(['theme', 'plugin'])
const SLUG_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/
const ID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i

type InstallInstruction = { label: string, code: string }
type AppSettings = { siteURL: string, adminURL: string, gateway: string }
...
const readString = (
  value: unknown,
  label: string,
  options: { required?: boolean, max: number } = { max: 160 },
) => {
...
const readUrl = (value: unknown, label: string, required = false) => {
  const normalized = readString(value, label, { required, max: 2000 })
...
const readSettings = (value: unknown): AppSettings => {
...
const readInstalls = (value: unknown): InstallInstruction[] => {
  if (value === undefined) return []
  if (!Array.isArray(value) || value.length > 50) badRequest('installs must be an array with at most 50 items')
  return (value as unknown[]).map((item, index) => {
    const source = requireObject(item, `installs[${index}]`)
    return {
      label: readString(source.label, `installs[${index}].label`, { required: true, max: 120 }),
      code: readString(source.code, `installs[${index}].code`, { required: true, max: 4000 }),
    }
  })
}

const parseInput = (value: unknown): AppInput => {
  const source = requireObject(value, 'body')
  const types = readString(source.types, 'types', { required: true, max: 16 })
  if (!VALID_TYPES.has(types)) badRequest('types must be theme or plugin')
  ...
  return {
    ...
    status: status as AppInput['status'],
    uniqueName: readString(source.uniqueName, 'uniqueName', { required: true, max: 190 }),
    packageUrl: readUrl(source.packageUrl, 'packageUrl', true),
    version: readString(source.version, 'version', { required: true, max: 80 }),
    installs: readInstalls(source.installs),
...
const selectRows = async (query: SQL): Promise<CatalogRow[]> => {
  const result = resolveExtensionDatabaseDialect() === 'sqlite'
    ? await db.all(query)
    : await db.execute(query)
  return normalizeRows(result)
}

const mutate = async (query: SQL) => resolveExtensionDatabaseDialect() === 'sqlite'
  ? db.run(query)
  : db.execute(query)
...
const toItem = (row: CatalogRow) => ({
  ...
  types: String(row.types) as AppInput['types'],
  ...
  status: Number(row.status) as AppInput['status'],
  uniqueName: String(row.unique_name),
  packageUrl: String(row.package_url),
  version: String(row.version),
  installs: parseJson<InstallInstruction[]>(row.installs_json, []),
  settings: parseJson<AppSettings>(row.settings_json, { siteURL: '', adminURL: '', gateway: '' }),
  seo: {
...
export const listApps = async (event: H3Event) => {
  ...
  const where = sql`
    WHERE (${rawStatus} = 0 OR status = ${rawStatus})
      AND (${search} = '' OR LOWER(name) LIKE LOWER(${search}) OR LOWER(slug) LIKE LOWER(${search}) OR LOWER(unique_name) LIKE LOWER(${search}))
  `
...
export const getApp = async (event: H3Event) => {
  const item = await findById(parseId(getQuery(event).id))
  if (!item) throw createError({ statusCode: 404, message: 'App not found' })
  return item
}

export const createApp = async (event: H3Event) => {
  ...
  try {
    await mutate(sql`
      INSERT INTO ${sql.identifier(TABLE)} (
        ...
        ${id}, ${input.name}, ${input.types}, ${input.slug}, ${input.imageUrl || null}, ${input.subtitle},
        ${input.priceAmount}, ${input.content}, ${input.downs}, ${input.status}, ${input.uniqueName},
        ${input.packageUrl}, ${input.version}, ${JSON.stringify(input.installs)}, ${JSON.stringify(input.settings)},
        ${input.seo.title}, ${input.seo.keywords}, ${input.seo.description}, ${now}, ${now}
      )
    `)
  } catch (error) {
...
export const updateApp = async (event: H3Event) => {
  const body = requireObject(await readBody(event), 'body')
  const id = parseId(body.id)
  if (!await findById(id)) throw createError({ statusCode: 404, message: 'App not found' })
  ...
  try {
    await mutate(sql`
      UPDATE ${sql.identifier(TABLE)} SET
        name = ${input.name}, types = ${input.types}, slug = ${input.slug}, image_url = ${input.imageUrl || null},
        subtitle = ${input.subtitle}, price_amount = ${input.priceAmount}, content = ${input.content},
        downs = ${input.downs}, status = ${input.status}, unique_name = ${input.uniqueName},
        package_url = ${input.packageUrl}, version = ${input.version}, installs_json = ${JSON.stringify(input.installs)},
        settings_json = ${JSON.stringify(input.settings)}, seo_title = ${input.seo.title},
...
export const deleteApp = async (event: H3Event) => {
  ...
  if (existing.status === STATUS_PUBLISHED) {
    throw createError({ statusCode: 409, message: 'Published apps must be changed to draft before deletion' })
  }
